...
const TextWrapper = (props) => {
    const handleMouseUp = () => {
        ...
        const annotations = [...props.value, { start, end, text: content.slice(start, end), highlight: props.highlight }];
    ...
    return (_jsx(Selection, { onMouseUp: handleMouseUp, backgroundColor: props.highlight?.color, children: splits.map((split) => (_jsx(Splitter, { ...split, onClick: handleSplitClick }, `${split.start}-${split.end}`))) }));
};
